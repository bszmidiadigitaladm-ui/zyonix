...
import { downloadPosterPng } from "@/lib/posters/client";
      ...
      {posters.map((poster) => {
        ...
        return (
          <div key={poster.id} className="overflow-hidden rounded-lg border border-border bg-surface">
            {ready ? (
              <PosterImage src={poster.image_url!} className="rounded-none" />
            ) : (
            ...
            <div className="flex flex-col gap-1 p-2 text-xs text-muted">
              <span className="truncate">{template?.name ?? poster.template_slug}</span>
              {ready && (
                <span className="flex flex-wrap gap-x-3 gap-y-1">
                  <button
                    type="button"
                    onClick={async () => {
                      const ok = await downloadPosterPng(poster.image_url!, `${poster.template_slug}.png`);
                  ...
                  <Link href={`/templates/${poster.template_slug}`} className="font-medium text-accent hover:underline">
                    {t("useAgain")}
                  </Link>
                </span>
              )}
